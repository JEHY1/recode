...
public class AdminQnAController {
    ...
    public String deleteAdminQna(@PathVariable Long QnAId, RedirectAttributes rttr, Integer page, Integer category, String searchKeyword) throws UnsupportedEncodingException {
        ...
        return "redirect:/admin/qna/index?page=" + page + "&category=" + category + "&searchKeyword=" + encodeSearchKeyword;
    }
    ...
    public String indexAdminQna(Model model, @PageableDefault(page = 0, size = 10, sort = "qnAId", direction = Sort.Direction.DESC) Pageable pageable, @RequestParam(required = false) Integer isDel, @RequestParam(defaultValue = "0") Integer category, @RequestParam(defaultValue = "") String searchKeyword) {
        ...
        if(searchKeyword == null || searchKeyword.equals("")) { // 검색 안했을 때
            qnAList = qnAService.qnAViewList(pageable);
        }
        else { // searchKeyword 로 검색 됐을 때
            qnAList = qnAService.qnAViewSearchList(category, searchKeyword, pageable);
        }
