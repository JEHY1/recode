$(function() {
    ...
    if(url.get("isDel") == 1) {
        history.pushState(null, null, "/admin/review/index?page=" + url.get("page") + "&category=" + url.get("category") + "&searchKeyword=" + url.get("searchKeyword"));
    }
    // 검색한 searchKeyword -> URL 로 보내기
    $("#selectBox > button").on("click", function() {
        location.href = "/admin/review/index?page=0&category=" + $("#selectBox > select").val() + "&searchKeyword=" + $("#selectBox > input").val();
    });
