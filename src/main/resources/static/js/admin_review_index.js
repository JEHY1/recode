            ...
            success : function(data) {
                if (data == "delete") {
                    ...
                    if($("#pageBox > a:last-child").prop("href") == "javascript:void(0)") {
                        location.href = "/admin/review/index?isDel=1&page=" + ($(".check:checked").length == $(".check").length ? page - 1 : page); // 싹다 체크 됐을 때 전 페이지로
                    }
                    else {
                        location.href = "/admin/review/index?isDel=1&page=" + page;
                    }
                }
            }
        });
